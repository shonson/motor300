...
uint8_t g_rx_data[25] = {0};//g_rx_data[20] for GUI 2.0
//Similarly for tx
...
uint32_t g_rx_complete_status = 1u;
...
uint8_t hid_conf_descr[HID_CONFIG_DESCR_LENGTH] = {
    ...
    0x03u,                                              /* bmAttributes */
    0x19u,                                              /* wMaxPacketSize LSB */ //x19 for GUI 2.0
    0x00u,                                              /* wMaxPacketSize MSB */
    0x08u                                               /* bInterval */
};
...
)
{
	...
	if(g_rx_complete_status ==1)
	{
		MSS_USBD_HID_rx_report(25); // Modified by Sudeep --> 25 for GUI 2.0
		USBMsgHandler();
...
)
{
    if(g_hid_state == USBD_HID_CONFIGURED)
    {
        MSS_USBD_rx_ep_read_prepare(MSS_USB_RX_EP_1, (uint8_t*)&g_rx_data, length);
        return (1u);
    }
    else
    {
