 ...
 *
 * SVN $Revision: 6939 $
 * SVN $Date: 2014-10-07 15:37:49 +0530 (Tue, 07 Oct 2014) $
 */
...
typedef struct {
    void(*usb_host_tx_complete)(uint8_t ep_num, uint8_t status);
    void(*usb_host_rx)(uint8_t ep_num, uint8_t status);
    void(*usb_host_cep)(uint8_t status);
    void(*usb_host_sof)(uint32_t frame_number);
    void(*usb_host_connect)(mss_usb_device_speed_t target_speed,
                            mss_usb_vbus_level_t vbus_level);

    void(*usb_host_disconnect)(void);
    void(*usb_host_vbus_error)(mss_usb_vbus_level_t);
    void(*usb_host_babble_error)(void);
    void(*usb_host_session_request)(void);
} mss_usb_host_cb_t;
...
/***************************************************************************//**
  @brief MSS_USB_HCIF_handle_connect_irq()
...
/***************************************************************************//**
  @brief MSS_USB_HCIF_cep_configure()
...
/***************************************************************************//**
  @brief MSS_USB_HCIF_tx_ep_configure()
...
/***************************************************************************//**
  @brief MSS_USB_HCIF_rx_ep_configure()
...
/***************************************************************************//**
  @brief MSS_USB_HCIF_bus_suspend()
...
/***************************************************************************//**
  @brief MSS_USB_HCIF_read_vbus_level()
...
/***************************************************************************//**
  @brief MSS_USB_HCIF_ep_tx_start()
...
*/
void
MSS_USB_HCIF_ep_tx_start
(
    mss_usb_ep_t* host_ep
);

/***************************************************************************//**
  @brief MSS_USB_HCIF_tx_ep_mp_configure()
...
/***************************************************************************//**
  @brief MSS_USB_HCIF_rx_ep_mp_configure()
...
/***************************************************************************//**
  @brief MSS_USB_HCIF_cep_write_pkt()
...
/***************************************************************************//**
  @brief MSS_USB_HCIF_cep_start_xfr()
...
*/
void
MSS_USB_HCIF_cep_start_xfr
(
    mss_usb_ep_state_t state
);

/***************************************************************************//**
  @brief MSS_USB_HCIF_cep_abort_xfr()
...
/***************************************************************************//**
  @brief MSS_USB_HCIF_cep_read_pkt()
...
/***************************************************************************//**
  @brief MSS_USB_HCIF_rx_ep_read_pkt()
...
/***************************************************************************//**
  @brief MSS_USB_HCIF_tx_ep_write_pkt()
...
/***************************************************************************//**
  @brief MSS_USB_HCIF_rx_ep_read_prepare()
...
/*******************************************************************************
 Exported Static __Inline functions
 */
static __INLINE void MSS_USB_HCIF_assert_bus_reset(void)
{
    USB->POWER |= POWER_REG_BUS_RESET_SIGNAL_MASK;
}

static __INLINE void MSS_USB_HCIF_clr_bus_reset(void)
{
    USB->POWER &= ~POWER_REG_BUS_RESET_SIGNAL_MASK;
}

static __INLINE void MSS_USB_HCIF_assert_bus_resume(void)
{
    USB->POWER |= POWER_REG_RESUME_SIGNAL_MASK;
    /*clear after min. 20ms*/
}

static __INLINE void MSS_USB_HCIF_clr_bus_resume(void)
{
    USB->POWER &= ~POWER_REG_RESUME_SIGNAL_MASK;
}

static __INLINE void MSS_USB_HCIF_enable_suspendm_out(void)
{
    USB->POWER |= POWER_REG_ENABLE_SUSPENDM_MASK;
}

static __INLINE void MSS_USB_HCIF_disable_suspendm_out(void)
{
    USB->POWER &= ~POWER_REG_ENABLE_SUSPENDM_MASK;
}

static __INLINE void MSS_USB_HCIF_assert_suspend_bus(void)
{
    USB->POWER |= POWER_REG_SUSPEND_MODE_MASK;
}

static __INLINE void MSS_USB_HCIF_clr_suspend_bus(void)
{
    USB->POWER &= ~POWER_REG_SUSPEND_MODE_MASK;
}

static __INLINE uint8_t MSS_USB_HCIF_is_host_suspended(void)
{
    return (((USB->POWER & POWER_REG_SUSPEND_MODE_MASK) ? MSS_USB_BOOLEAN_TRUE
                                                        : MSS_USB_BOOLEAN_FALSE));
}

static __INLINE void MSS_USB_HCIF_end_session(void)
{
 ...
 */
static __INLINE uint8_t MSS_USB_HCIF_is_target_ls(void)
{
    return (((USB->DEV_CTRL & DEV_CTRL_LS_DEV_MASK) ? MSS_USB_BOOLEAN_TRUE
                                                    : MSS_USB_BOOLEAN_FALSE));
}

static __INLINE uint8_t MSS_USB_HCIF_is_target_fs(void)
{
    return (((USB->DEV_CTRL & DEV_CTRL_FS_DEV_MASK) ? MSS_USB_BOOLEAN_TRUE
                                                    : MSS_USB_BOOLEAN_FALSE));
}

static __INLINE uint8_t MSS_USB_HCIF_is_hs_mode(void)
{
...
MSS_USB_HCIF_cep_clr_rxpktrdy(void)
{
    USB->ENDPOINT[MSS_USB_CEP].TX_CSR &= ~CSR0L_HOST_RX_PKT_RDY_MASK;
}

static __INLINE void MSS_USB_HCIF_cep_set_request_in_pkt(void)
{
    /*Cleared when RxPktRdy is cleared*/
    USB->ENDPOINT[MSS_USB_CEP].TX_CSR |= CSR0L_HOST_IN_PKT_REQ_MASK;
}

static __INLINE void MSS_USB_HCIF_cep_set_setuppktonly(void)
{
    USB->ENDPOINT[MSS_USB_CEP].TX_CSR |= (CSR0L_HOST_SETUP_PKT_MASK);
}
static __INLINE void MSS_USB_HCIF_cep_set_setuppktrdy(void)
{
    USB->ENDPOINT[MSS_USB_CEP].TX_CSR |= (CSR0L_HOST_SETUP_PKT_MASK |
                                              CSR0L_HOST_TX_PKT_RDY_MASK);
}

static __INLINE void MSS_USB_HCIF_cep_set_statuspktrdy_after_out(void)
{
    USB->ENDPOINT[MSS_USB_CEP].TX_CSR |= (CSR0L_HOST_STATUS_PKT_MASK |
                                              CSR0L_HOST_IN_PKT_REQ_MASK);
}

static __INLINE void MSS_USB_HCIF_cep_set_statuspktrdy_after_in(void)
{
    USB->ENDPOINT[MSS_USB_CEP].TX_CSR |= (CSR0L_HOST_STATUS_PKT_MASK |
                                              CSR0L_HOST_TX_PKT_RDY_MASK);
}

static __INLINE void MSS_USB_HCIF_cep_clr_statusRxpktrdy(void)
{
...
static __INLINE uint16_t MSS_USB_HCIF_cep_read_csr_reg(void)
{
    return(USB->ENDPOINT[MSS_USB_CEP].TX_CSR);
}

static __INLINE void MSS_USB_HCIF_cep_set_txpktrdy(void)
{
    USB->ENDPOINT[MSS_USB_CEP].TX_CSR |= CSR0L_HOST_TX_PKT_RDY_MASK;
}

static __INLINE void
MSS_USB_HCIF_cep_clr_data_tog
(
    void
)
{
...
static __INLINE void
MSS_USB_HCIF_cep_set_data_tog_we
(
    void
)
{
...
static __INLINE void
MSS_USB_HCIF_cep_clr_data_tog_we
(
    void
)
{
 ...
 */
static __INLINE void MSS_USB_HCIF_load_tx_fifo(mss_usb_ep_num_t ep_num,
                                               void * in_data,
                                               uint32_t length)
{
    uint32_t idx;
...
static __INLINE void MSS_USB_HCIF_read_rx_fifo(mss_usb_ep_num_t ep_num,
                                               void * out_data,
                                               uint32_t length)
{
    uint32_t idx;
 ...
 */
static __INLINE void
MSS_USB_HCIF_tx_ep_set_target_func_addr
(
 ...
 */
static __INLINE void
MSS_USB_HCIF_cep_set_type0_reg
(
    mss_usb_device_speed_t speed
)
{
...
MSS_USB_HCIF_cep_get_type0_reg
(
    void
)
{
