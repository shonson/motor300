...
static uint8_t g_bot_inquiry[36] __attribute__ ((aligned (4))) = {0};          //Store Inquiry response
static uint8_t g_bot_readcap[8] __attribute__ ((aligned (4)))= {0};            //Store Read_capacity response
...
)
{
    ...
    switch(g_msc_bot_state)
    {
        ...
        if(g_scsi_command.dbuf_len ==0)//zdl request
        {
        ...
        else
        {
            ...
            if(g_scsi_command.cbuf[12] & 0x80)  //bmCBWFLags field -- Read Command
            {
                MSS_USBH_read_in_pipe(g_msd_tdev_addr,
                                      USBH_MSC_BULK_IN_PIPE,
                                      g_tdev_in_ep.num,
        ...
        g_scsi_command.dbuf_len -= count;
        if(g_scsi_command.dbuf_len ==0)//Still there is data to be transferred.
        {
            g_msc_bot_state = MSC_BOT_STATUS_WAITCOMPLETE;
