 ...
 *
 * SVN $Revision: 7025 $
 * SVN $Date: 2014-11-14 12:59:40 +0530 (Fri, 14 Nov 2014) $
 */
/*=========================================================================*//**
  ...
  interface. Once initialized the USBH driver is ready to detect the attach event
  on the USB port. The application call-back interface is used to provide
  ...
  the USBH-Class driver will be informed about it by calling a call-back function.
  Please refer the USBH-Class Interface section above for more details.

  This driver supports multi-packet Bulk transfer (both IN and OUT), to make sure
  that it is able to use the full bandwidth offered by USB Bulk transfers. When
  a transfer size of more than wMaxPktSize is provided to this driver using
  MSS_USBH_write_out_pipe()  function or MSS_USBH_read_in_pipe() function, this
  driver handles this muti-packet transfer internally by dividing it into
  multiple Bulk transfers of wMaxPktSize each. This way no application
  intervention is required till the total multi-packet Bulk transfer is complete.

  The USBH driver supports Bulk transfers using the DMA inside MSS USB or without
  using the internal DMA.   It is advised that the USBH driver is configured to
  ...
  USBH driver must be modulo-4. This is not a requirement when the MSS USB driver
  is configured not to use internal DMA.

  All the control transfers happen on the control pipe. The control pipe is
  shared by the USBH driver and the USBH-Class driver. When the USBH-Class driver
  needs to perform control transfer on the attached device, it must first
  configure the control pipe using the MSS_USBH_configure_control_pipe() function.
  The  MSS_USBH_start_control_xfr() function can be used to start the control
  transfer After the control pope is configured. The data direction is determined
  by a parameter provided with this function. The USBH-Class driver will be
  informed about the data transfer completion or the error status by calling a
  call-back function. Please refer the USBH-Class Interface section above for
  more details. The control transfer can be started by a USBH-Class driver only
  when the control pipe is free.The MSS_USBH_get_cep_state() function can be
  used to know the current state of the control pipe. At the start of the control
  transfer the USBH driver must send a setup-packet to the attached device in
  the setup phase. The function MSS_USBH_construct_get_descr_command() can be
  used to construct the setup packet as per the USB specification for the
  transfer to be performed. This function formats the setup packet depending on
  the parameters provided with this function.
...
/*
 The value received in the bDescriptorType field in device descriptor is invalid.
*/
#define DEV_DESC_WRONG_DESC_TYPE                        -2

/*
 The value received in the bLength field of the device descriptor does not match
 the standard value (18).
*/
...
dev_desc_t
  ...
  bDeviceProtocol
  The bDeviceProtocol parameter indicates the standard USB data transfer protocol
  supported by the device. The meaning of this parameter is exactly as defined
...
def_conf_desc_t
  ...
  descriptor information. This might be useful to retrieve some basic information
...
mss_usbh_class_cb_t
  ...
  a class driver with the the USBH driver. This parameter must be of the form:
    ...
    0x00        USB Std class code    USB Std sub-class code    USB Std Protocol code
  ...
  usbh_class_dataout_done
  The function pointed by the usbh_class_dataout_done element will be called
  ...
  usbh_class_datain_done
  The function pointed by the usbh_class_datain_done element will be called
...
*/
typedef struct mss_usb_host_class_cb {
    uint32_t class_id;
    ...
    uint8_t (*usbh_class_dataout_done)(uint8_t tdev_addr,
                                       uint8_t status,
                                       uint32_t count); //add epnum to parameter

    uint8_t (*usbh_class_datain_done)(uint8_t tdev_addr,
                                      uint8_t status,
                                      uint32_t count);
...
mss_usbh_tdev_info_t
  ...
  attached supports multiple transaction translators. This parameter is meaningful
  only when Multi-Point support is enabled. Multi-point support is currently not
  enabled in the USBH driver.
...
mss_usbh_user_cb_t
  ...
  error_code parameter provides the exact reason why the device is not supported.
...
/*******************************************************************************
 * EXPORTED APIs from USBH driver
 *******************************************************************************/
...
/***************************************************************************//**
  ...
  mss_usbh_user_cb_t. This structure is used to call the user call-back functions
  by the USBH driver. These call-back functions can be used to provide
  error/status messages to the user for the application specific handling of
  the events.
...
/***************************************************************************//**
  @brief MSS_USBH_register_class_driver()
  ...
  @param class_handle
  The class_handle parameter specifies the class driver handle to be registered
...
/***************************************************************************//**
  @brief MSS_USBH_configure_control_pipe()
...
/***************************************************************************//**
  @brief MSS_USBH_configure_in_pipe()
  ...
  @param fifo_addr
  The fifo_addr parameter is the address of the FIFO in the MSS USB internal RAM.
  The valid FIFO address values are from 0x00 to 0xFFF8. The FIFO Address must
  be a multiple of 8. If the value provided is not a multiple of 8, then the
  immediate lower value which is a multiple of 8 is taken as the FIFO address.
  ...
  transfer. This value must be equal to the maximum packet size as mentioned in
  the endpoint descriptor which is used during enumeration process.
  ...
  This parameter represents the number of packets transferred in one (micro) frame.

  High bandwidth ISO transfers – This parameter represents the number of packets
  transferred in one (Micro) frame. In this case, this parameter can have a
  value of 1 2 or 3. High bandwidth ISO transfers are not yet implemented.
  ...
  transfer. The auto-amalgamation/auto-split feature is implemented but not yet
  tested.
  ...
  A unique DMA channel must be selected to transfer data on individual endpoints.
  ...
  if the transfer size is exact a multiple of wMaxPacketSize. This parameter is
  ...
  pipe must timeout on receiving a stream of NAK responses. The valid values for
  this parameter are as mentioned in the table below.

    Transfer Type            Speed                            Valid values
                                                    (No of frames/microframes)
    Interrupt                LS or FS                        1 – 255
                                HS                1,2,4,8,16,32,64…32768 microframes

    Isochronous              FS or HS             1,2,4,8,16,32,64…32768 frames/microframes

    Bulk                     FS or HS             1,2,4,8,16,32,64…32768 frames/microframes
...
/***************************************************************************//**
  @brief MSS_USBH_configure_out_pipe()
  ...
  @param outpipe_num
  The outpipe_num parameter indicates the OUT pipe number (endpoint) on MSS USB
  which must be used for OUT transfers with the attached device.
  ...
  Bulk transfers. – For Bulk transfer this value must always be ‘1u’. This
  parameter will be used with the auto-amalgamation/auto-split feature where it
  indicates the number of bulk packets to be auto-amalgamated/auto-split in bulk
  transfer. The auto-amalgamation/auto-split feature is implemented but not yet
  tested.
  ...
  endpoint. A unique DMA channel must be selected to transfer data on individual
  ...
  @param add_zlp
  The add_zlp parameter indicates whether to send a zero length packet (ZLP) if
  the transfer size is a exact multiple of wMaxPacketSize. This parameter is
  only applicable for Bulk transfers. For all other transfer types this
  parameter is ignored.

  @param interval
  The interval parameter for interrupt and isochronous transfers, defines the
  polling interval for the currently-selected OUT pipe. For the bulk transfers,
  this parameter indicates the number of frames/microframes after which the OUT
  Pipe must time out on receiving a stream of NAK responses. The valid values for
  this parameter are as mentioned in the table below.
...
/***************************************************************************//**
  ...
  this transfer after the status phase of the current OUT transaction is complete.

  @param target_addr
  The target_addr parameter is the address of the attached device with which the
  MSS USB needs to communicate.

  @param outpipe_num
  The outpipe_num parameter indicates the OUT pipe number (endpoint) on the MSS
  ...
  @param buf
  The buf parameter is the address of the buffer provided by the user from which
  the data is copied to the selected pipe buffer.
...
/***************************************************************************//**
  ...
  non-blocking. A call-back function will be called to indicate the status of
  this transfer after the status phase of the current IN transaction is complete.
  ...
  @param outpipe_num
  The outpipe_num parameter indicates the OUT pipe number (endpoint) on MSS USB
  which must be used for the OUT transfers with the attached device.
...
/***************************************************************************//**
  @brief MSS_USBH_get_tdev_state()
  The MSS_USBH_get_tdev_state() function can be used to find out the current
  state of the attached device. E.g. ADDRESS, CONFIGURED etc. (as defined by USB2.0).
...
/***************************************************************************//**
  ...
  @code
        ...
        // which might take time in 100s of milliseconds depending on target device.
        //
...
/***************************************************************************//**
  ...
  the number of milliseconds elapsed from the time when the last reset occurred.
  This integer value will overflow after approximately 49Hrs after reset.
...
/***************************************************************************//**
  @brief MSS_USBH_construct_get_descr_command()
  ...
  @param xfr_dir
  The xfr_dir parameter indicates the direction of the data transfer on the USB
  bus. The value USB_STD_REQ_DATA_DIR_IN indicates that the direction of the
  data transfer is from the USB device to USB host (USB IN transaction). The
  value USB_STD_REQ_DATA_DIR_OUT indicates that the direction of the data
  ...
  request recipient on the attached device can be one of the following: device,
  ...
  @param strng_idx
  The strng_idx parameter indicates the index of the string descriptor when the
  desc_type parameter indicates STRING TYPE.
...
/***************************************************************************//**
  @brief MSS_USBH_start_control_xfr()
  ...
  the control transfer. The USBD driver will put the data received from attached
  device in this buffer for the USB IN transfers. The data from this buffer will
  be sent to the attached device for the USB OUT transfers.
  ...
  that the direction of the data transfer is from the USB device to the USB host
  (USB IN transaction). The value USB_STD_REQ_DATA_DIR_OUT indicates that the
  direction of the data transfer is from the USB host to the USB device
  (USB OUT transaction).

  @param data_len
  The data_len parameter indicates the number of bytes to be transferred during
...
/***************************************************************************//**
  ...
  @return
  This function returns a value of type mss_usb_ep_state_t indicating the current
  state of the control pipe.
...
/***************************************************************************//**
  @brief MSS_USBH_test_mode()
  The MSS_USBH_test_mode() function can be used to initiate one of the compliance
  test modes in the MSS USB. This function is useful only for compliance test
  procedures. The MSS USB goes into the specified test mode once this function
  is executed. A hard reset is required to resume normal operations.

  @param test_case
  The test_case parameter indicates the compliance test that need to be executed.
  The compliance tests could be one of the following:
...
/***************************************************************************//**
  @brief MSS_USBH_get_std_dev_descr()
