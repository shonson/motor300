...
/*=========================================================================*//**
  ...
  authentication.
  The large SmartFusion2 devices starting from the M2S090 include an SRAM-PUF 
  ...
  secret from the SRAM-PUF start-up value. The start-up value of the PUF’s 
  SRAM block is slightly different from one power-up to the next. Some 
  processing is performed on the PUF’s SRAM start-up value to eliminate
  randomness and retrieve the exact same intrinsic secret on each power-up
  cycle. This processing is performed using the activation code.
  The activation code must be generated once, typically when the system 
  containing the SmartFusion2 device is being commissioned. The activation code
  is created using the following function:
    - MSS_SYS_puf_create_activation_code()
  The activation code is stored the System Controller’s private eNVM after
  being created. Its value is never exported in clear text from the System
  Controller.
  ...
  SmartFusion2 is being decommissioned or re-purposed.
  ...
  is retrieved by the user. A key code is stored in the System Controller’s
  private eNVM instead of the key’s value. The key code is generated when a
  key is enrolled. The key code value is created from the enrolled key’s
  value and the intrinsic secret value. The key’s value can then later be
  regenerated from the key code value and intrinsic secret value upon user
  request.
  ...
  Keys are identified by a number and must be enrolled sequentially. This means
  that the first step in enrolling a new key is to determine how many keys are
  ...
  Keys are numbered from 0 to 57. The number of keys returned by 
  MSS_SYS_puf_get_number_of_keys() can then be used as the newly enrolled key
  identification number since key numbering is zero based.
  ...
  eNVM during the export procedure.
  
  Retrieving the Design Security ECC Public Key
  The design security ECC public key can be retrieved using the following
  function:
  ...
  The tamper control services can be used to enable/disable clock monitoring, 
  to control power to PUF and to clear mesh short and lock parity tamper event
...
/*-------------------------------------------------------------------------*//**
  ...
  - TAMPER_CLOCK_ERROR_DETECT_OPCODE
...
/*-------------------------------------------------------------------------*//**
  ...
  @return
    ...
    NOTE: Private eNVM factory and user is available only on 090 device onwards.
 */
...
/*-------------------------------------------------------------------------*//**
  ...
  Note: This system services is only available on large SmartFusion2 device
        starting with the M2S090.
...
/*-------------------------------------------------------------------------*//**
  ...
  This service uses tamper control service with bit 1 in Options be set
  to 1. System controller will disable 50 MHZ timing clock monitoring and clock 
