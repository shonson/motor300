BLDC_with_Hall_Design Directory Structure

The BLDC_with_Hall_Design directory is organized as shown below:

 BLDC_with_Hall_Design 
     |
     |----BLDC_with_Hall_Design_Job_File
     |----src
     |     |---cfg
     |     |---components
     |     |---constraints
...
BLDC_with_Hall_Design Folders

BLDC_with_Hall_Design_df\softconsole2021p1 folder Contains the softconsole Project to run the Design

**** Note :- Remove the .hex file from the design when you want to run the softconsole in Debug Mode

BLDC_with_Hall_Design_Job_File folder contains Job File for the corresponding deisgn 
 
BLDC_with_Hall_Design\src folder contains the following folders:
...
BLDC_with_Hall_Design\src includes the following TCL files:
