 ...
 *
 * SVN $Revision: 6965 $
 * SVN $Date: 2014-10-13 16:56:26 +0530 (Mon, 13 Oct 2014) $
 */
/*=========================================================================*//**
  ...
  and one BULK OUT endpoint. This driver implements the USBD_printer_data_in()
  and the USBD_printer_data_out() function to get the information on data
  transfer events on the USB bus which are called by the USBD Driver.

  The USBD_printer_data_in() and USBD_printer_data_out() calls the call-back
  function where application can read the data received from USB HOST and return
  the post status and additional information to HOST.
*/
...
#define PRINTER_CLASS_BULK_OUT_EP            MSS_USB_RX_EP_1
#define PRINTER_CLASS_BULK_IN_EP             MSS_USB_TX_EP_1
...
/***************************************************************************//**
  ...
  usb_printer_deinit
  The function pointed to by the usb_printer_deinit function pointer is called on
  ...
  usb_printer_datain
  The function pointed by the usb_printer_datain function pointer is called by
  ...
  usb_printer_dataout
  The function pointed by the usb_printer_dataout function pointer is called on
  ...
  usb_printer_cep_datain
  The function pointed by the usb_printer_cep_datain function pointer is called
  ...
  usb_printer_cep_dataout
  The function pointed by the usb_printer_cep_dataout function pointer is called
...
typedef struct mss_usbd_printer_cb  {
    ...
    uint8_t(*usb_printer_deinit)(uint8_t cfgidx);
    ...
    uint8_t(*usb_printer_datain)(mss_usb_ep_num_t num, uint8_t status);
    uint8_t(*usb_printer_dataout)(mss_usb_ep_num_t num,
                                uint8_t status,
                                uint32_t rx_count);

    uint8_t(*usb_printer_cep_datain)(uint8_t status);
    uint8_t(*usb_printer_cep_dataout)(uint8_t status);
